...
kya = 8
frompthresh = 15000000
topthresh =   22000000
steppthresh =   500000

def clusterworld(adjlist,ix,productthresh,densities):
    n = len(adjlist)
    clustercount = 0
    clusters = []
    visited = []
...
def trythreshold(pottery,pthresh,kya):
    numquarters = 6084
    ix = int(numquarters-1 - kya*40)
    ya = (numquarters - ix)*25
    clusters = clusterworld(adjlist,ix,pthresh,densities)
...
for productthresh in range(frompthresh,topthresh,steppthresh):
    (score,matchcluster,intersect) = trythreshold(pottery,productthresh,kya)
    print(f'threshold: {productthresh}, hamming distance: {score}')
...
filename = f'best{bpt}.txt'
savehexagons(bcluster,filename)
caption = f'threshold:{bpt}'
